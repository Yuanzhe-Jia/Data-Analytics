...
data_0 as (
  ...
  where 
      ...
      and date(eventTimeMs) = "2023-12-28"
),
...
data_7 as (--devices in the initial period with login activities
  ...
  where 
      is_target = 1 
      and is_1_follow = "stctv_app.NewLoginVC" 
      and is_2_follow = "stctv_app.ProfileSelectionViewController" 
      and is_3_follow is not null 
      and is_3_follow <> "stctv_app.AppLaunchViewController"
  group by 1
),
...
data_8 as (--devices in the follow-up period with login activities
  ...
  where 
      title is not null 
      and title <> "" 
      and title = "stctv_app.NewLoginVC" 
      and date(eventTimeMs) > "2023-12-28"
  group by 1
),
